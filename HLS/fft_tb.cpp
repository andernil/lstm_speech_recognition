...
int main()
{
	...
	// Load golden log-mel-filterbank energies for reference
	ifstream goldenMels ("golden_mel.dat");
	int num_mels_read = 0;
	if(goldenMels.is_open())
	{
		for(int mel_row = 0; mel_row < NUM_FRAMES; mel_row++)
		{
			for(int mel_num = 0; mel_num < MEL_NUM_FILTERBANKS; mel_num++){
				getline(goldenMels, read_buffer, '\n');
	...
	cout << "Running log-mel-filterbank energy calculation" << endl;
	dut_main(test_data, energies, peak_val);
